...
type azureOpenai struct {
	...
	EndPoint         string  `json:"end_point" mapstructure:"end_point"`                 //终结点，在azure openai控制台查询
	DeploymentId     string  `json:"deployment_id" mapstructure:"deployment_id"`         //模型部署的名称
	ApiVersion       string  `json:"api_version" mapstructure:"api_version"`             //要用于此操作的 API 版本
	Model            string  `json:"model" mapstructure:"model"`                         //模型
	Temperature      float64 `json:"temperature" mapstructure:"temperature"`             //温度
	TopP             float64 `json:"top_p" mapstructure:"top_p"`                         //top_p
	MaxTokens        int     `json:"max_tokens" mapstructure:"max_tokens"`               //最大token
	Stop             string  `json:"stop" mapstructure:"stop"`                           //停止标志
	PresencePenalty  float64 `json:"presence_penalty" mapstructure:"presence_penalty"`   //presence_penalty
	FrequencyPenalty float64 `json:"frequency_penalty" mapstructure:"frequency_penalty"` //frequency_penalty
...
type azureTTS struct {
	ApiKey   string `json:"api_key" mapstructure:"api_key"`     //API密钥
	EndPoint string `json:"end_point" mapstructure:"end_point"` //终结点，在azure tts控制台查询
	Speak    struct {
		Version    string `json:"version" mapstructure:"version"`        //指示用于解释文档标记的 SSML 规范的版本。 当前版本为“1.0”。
		XmlLang    string `json:"xmlLang" mapstructure:"xml_lang"`       //根文档的语言。 该值可以包含语言代码，例如 en（英语），也可以包含区域设置，例如 en-US（美国英语）。
		XmlnsMstts string `json:"xmlnsMstts" mapstructure:"xmlns_mstts"` //xmlns:mstts
...
type azureSTT struct {
	ApiKey       string `json:"api_key" mapstructure:"api_key"`             //API密钥
	SpeechRegion string `json:"speech_region" mapstructure:"speech_region"` //语音区域
...
type memoryAndClean struct {
	//短期记忆token上限
	ShortMemoryTokenLimit int `json:"short_memory_token_limit" mapstructure:"short_memory_token_limit"` //短期记忆token清理限制

	//清理方式
	CleanAll bool `json:"clean_all" mapstructure:"clean_all"` //清理所有记忆
	CleanOne bool `json:"clean_one" mapstructure:"clean_one"` //清理一个记忆
}
